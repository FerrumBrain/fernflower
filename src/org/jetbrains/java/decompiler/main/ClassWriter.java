...
public class ClassWriter {
  ...
  public void classToJava(ClassNode node, TextBuffer buffer, int indent, BytecodeMappingTracer tracer) {
    ...
    try {
      ...
      for (StructMethod mt : cl.getMethods()) {
        ...
        var methodBuffer = new TextBuffer();
        boolean methodSkipped = !methodToJava(node, mt, methodBuffer, indent + 1, method_tracer);

        if (DecompilerContext.getOption(IFernflowerPreferences.OPTIMIZE_RECORDS)) {
          var defaultMethod = buildDefaultMethod(node, mt, indent + 1, components);
          if (components != null && Objects.equals(defaultMethod.toString(), methodBuffer.toString())) {
            methodSkipped = true;
          } else if (components != null && isConstructor(mt.getName()) && isCompactConstructor(defaultMethod.toString(), methodBuffer.toString())) {
            methodBuffer = buildCompactConstructor(defaultMethod.toString(), methodBuffer.toString(), indent + 1);
          }
        }
  ...
  private boolean isConstructor(String name) {
    return CodeConstants.INIT_NAME.equals(name);
  }

  private boolean isCompactConstructor(String defaultConstructor, String constructor) {
    String lineSeparator = new TextBuffer().appendLineSeparator().toString();
    var defaultLines = List.of(defaultConstructor.split(lineSeparator));
    var lines = List.of(constructor.split(lineSeparator));
    for (int i = 1; i < defaultLines.size(); i++) {
      if (!Objects.equals(lines.get(lines.size() - defaultLines.size() + i), defaultLines.get(i))) {
        return false;
      }
    }
    return lines.contains(defaultLines.get(0));
  }

  private TextBuffer buildCompactConstructor(String defaultConstructor, String constructor, int indent) {
    String lineSeparator = new TextBuffer().appendLineSeparator().toString();
    var defaultLines = List.of(defaultConstructor.split(lineSeparator));
    var lines = List.of(constructor.split(lineSeparator));
    int defaultIndex = 0;
    ...
    for (String line : lines) {
      if (Objects.equals(line, defaultLines.get(defaultIndex))) {
        if (defaultIndex == 0) {
          res.append(line.split("\\(")[0]);
          res.append(line.split("\\)")[1]).appendLineSeparator();
        }
        defaultIndex++;
        continue;
      }
      res.append(line).appendLineSeparator();
    }
    res.appendIndent(indent).append("}").appendLineSeparator();
    return res;
  }

  private TextBuffer buildDefaultConstructor(String name, List<String> argsTypes, List<String> argsNames, int indent, int flags, boolean isInterface) {
    TextBuffer res = new TextBuffer();
    res.appendIndent(indent);
    appendModifiers(res, flags, METHOD_ALLOWED, isInterface, METHOD_EXCLUDED);
    res.append(String.format("%s(", name));
    for (int i = 0; i < argsNames.size(); i++) {
      if (i != 0) {
        res.append(", ");
      }
      res.append(String.format("%s %s", argsTypes.get(i), argsNames.get(i)));
    }
    res.append(") {").appendLineSeparator();
    for (String argName : argsNames) {
      res.appendIndent(indent + 1).append(String.format("this.%s = %s;", argName, argName)).appendLineSeparator();
    }
    res.appendIndent(indent).append("}").appendLineSeparator();
    return res;
  }

  private TextBuffer buildDefaultGetter(String type, String name, int indent, int flags, boolean isInterface) {
    TextBuffer res = new TextBuffer();
    res.appendIndent(indent);
    appendModifiers(res, flags, METHOD_ALLOWED, isInterface, METHOD_EXCLUDED);
    res.append(String.format("%s %s() {", type, name)).appendLineSeparator();
    res.appendIndent(indent + 1).append(String.format("return this.%s;", name)).appendLineSeparator();
    res.appendIndent(indent).append("}").appendLineSeparator();
    return res;
  }

  private TextBuffer buildDefaultMethod(ClassNode node, StructMethod mt, int indent, List<StructRecordComponent> components) {
    if (components == null) {
      return new TextBuffer();
    }

    var argsTypes = new ArrayList<String>();
    var argsNames = new ArrayList<String>();
    for (int i = 0; i < components.size(); i++) {
      StructRecordComponent cd = components.get(i);
      boolean varArgComponent = i == components.size() - 1 && isVarArgRecord(node.classStruct);
      var argBuffer = new TextBuffer();
      recordComponentToJava(cd, argBuffer, varArgComponent);
      argsTypes.add(argBuffer.toString().split(" ")[0]);
      argsNames.add(argBuffer.toString().split(" ")[1]);
    }
    ...
    boolean isInterface = node.getWrapper().getClassStruct().hasModifier(CodeConstants.ACC_INTERFACE);

    if (isConstructor(mt.getName())) {
      return buildDefaultConstructor(node.simpleName, argsTypes, argsNames, indent, flags, isInterface);
    } else {
      for (int i = 0; i < argsNames.size(); i++) {
        if (Objects.equals(mt.getName(), argsNames.get(i))) {
          return buildDefaultGetter(argsTypes.get(i).replace("...", "[]"), argsNames.get(i), indent, flags, isInterface);
        }
      }
    }
    return new TextBuffer();
